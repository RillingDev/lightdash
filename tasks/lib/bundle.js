 ...
 * @returns {Promise}
 */
const createBundle = (format, bundle) =>
    new Promise((resolve, reject) => {
        bundle
            ...
            .then(result => {
                fs.writeFile(
                    path.join(
                        CONSTANTS.dirBase.output,
                        `${CONSTANTS.js.namespace.file}${format.ext}.js`
                    ),
                    result.code,
...
module.exports = function(formats, plugins) {
    rollup
        ...
        .then(bundle => {
            Promise.all(formats.map(format => createBundle(format, bundle)))
